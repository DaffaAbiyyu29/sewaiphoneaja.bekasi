...
export default function CreateUserPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!imageFile) newErrors.photo = "Foto profil wajib diunggah.";
    ...
    try {
      ...
      if (res?.data?.success) {
        setResponse({ success: true, message: "User berhasil dibuat." });
        setTimeout(() => (window.location.href = "/menu/user"), 1400);
      } else {
        setResponse({
          success: false,
          message: res?.data?.message || "Gagal membuat user.",
        });
      }
    } catch (err) {
      console.error(err);
          ...
          <div className="bg-gray-50 px-8 py-6 flex justify-end gap-3 border-t border-gray-200">
            ...
            >
              {loading ? (
                ...
                <>
                  <FontAwesomeIcon icon={faFloppyDisk} /> Simpan User
                </>
              )}
            </button>
          </div>
